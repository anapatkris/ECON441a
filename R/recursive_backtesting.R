...
#'
#' @examples
#'  recursive_backtesting(ex_model,ex_ts,k=100)
#'
recursive_backtesting<-function(model,ts,k){
  ...
  for (i in 1:(n-k0-1)) {
    trmodel<-Arima(ts[1:k1],model=model) #est arima on train
  ...
  plot(result$data,main='Acutual VS Predict')
